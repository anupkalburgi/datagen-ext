...
from datagen_ext.models import (
    DatagenSpec,
    TableDefinition,
    ColumnDefinition,
    FilePathTarget,
...
def test_prepare_data_generators(spark_session, sample_config):
    ...
    generator = Generator(spark=spark_session)
    prepared_generators, success = generator.prepare_data_generators(sample_config)
...
def test_prepare_data_generators_with_invalid_config(spark_session):
    ...
    with pytest.raises(ValueError, match="PK 'id' in table 'test_table' missing type"):
        generator.prepare_data_generators(invalid_config)
...
@patch("datagen_ext.generator.Generator.write_prepared_data")
