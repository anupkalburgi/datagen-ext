...
class Generator:
    ...
    def prepare_data_generators(
        ...
        for table_name in generation_order:
            if table_name not in tables_config:
                logger.warning(
                    f"Table '{table_name}' listed in generation order but not found in tables config. Skipping."
                )
            ...
            try:
                ...
                logger.warning(
                    f"Could not get default Spark parallelism for table '{table_name}', defaulting to 4. Error: {ex}"
                )
            ...
            try:
                ...
                for col_def in columns_spec:
                    ...
                    if is_primary:
                        ...
                        logger.info(
                            f"Table '{table_name}': Column '{col_name}' is PRIMARY KEY. Deriving from '{INTERNAL_ID_COLUMN_NAME}'."
                        )
                        ...
                        if col_type == "string":
                            ...
                            logger.debug(
                                f"PK '{col_name}' (string) using base '{INTERNAL_ID_COLUMN_NAME}', format '{pk_kwargs['format']}'"
                            )
                        ...
                        for opt_key in conflicting_opts_for_pk:
                            if opt_key in pk_kwargs:
                                logger.debug(
                                    f"For PK '{col_name}', option '{opt_key}' may be ignored due to baseColumn logic."
                                )

                        col_omit = getattr(col_def, "omit", None)
                        if col_omit is not None:
                            pk_kwargs["omit"] = col_omit
                        final_kwargs = pk_kwargs
                    else:
                        regular_kwargs = getattr(col_def, "options", {}).copy()
                        if col_type:
                            regular_kwargs["colType"] = col_type
                        base_col = getattr(col_def, "baseColumn", None)
                        if base_col:
                            regular_kwargs["baseColumn"] = base_col
                        base_col_type = getattr(col_def, "baseColumnType", None)
                        if base_col_type:
                            regular_kwargs["baseColumnType"] = base_col_type
                        col_omit = getattr(col_def, "omit", None)
                        if col_omit is not None:
                ...
                if is_critical_pk_error(ve):
                    logger.error(
                        f"Critical PK configuration error for table '{table_name}': {ve}. Halting preparation."
                    )
                    raise  # Re-raise critical PK ValueErrors to be caught by the caller
                else:
                    # Handle other ValueErrors locally
                    logger.exception(
                        f"--- Handled ValueError during DataGenerator spec preparation for table '{table_name}' ---"
                    )
                    prepared_generators[table_name] = None
                    all_specs_successful = False
            except Exception as e:
                logger.exception(
                    f"--- General ERROR during DataGenerator spec preparation for table '{table_name}' ---"
                )
                prepared_generators[table_name] = None
                all_specs_successful = False
        ...
        if (
            ...
            logger.info(
                f"--- DataGenerator spec preparation completed successfully for config: {config_source_name} ---"
            )
    ...
    def write_prepared_data(
        ...
        if not prepared_generators:
            logger.warning(
                f"No prepared data generators to write for config: {config_source_name}. Skipping write phase."
            )
            return True
        ...
        for table_name, data_gen in prepared_generators.items():
            if data_gen is None:
                logger.warning(
                    f"Skipping write for table '{table_name}' as its DataGenerator spec was not prepared successfully."
                )
                continue
            ...
            try:
                ...
                logger.info(
                    f"Table '{table_name}': Built DataFrame with {actual_row_count} rows (requested: {requested_rows})."
                )

                if actual_row_count == 0 and requested_rows > 0:
                    logger.warning(
                        f"Table '{table_name}': Requested {requested_rows} rows but built 0. Check definitions."
                    )
    ...
    def generate_and_write_data(
        ...
        if not prepared_generators_map and list(
            config.tables.keys()
        ):  # Check if tables were expected but none prepared
            logger.warning(
                f"No data generators were successfully prepared for config '{config_source_name}', though tables were defined. Nothing to write."
            )
